...
<script setup lang="ts">
  ...
  onMounted(async () => {
    ...
    try {
      const response = await fetch(`${config.public.fromTheHartAPIBaseUrl}/auth/verify-email`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok && data.data?.verified) {
        ...
        errorMessage.value = data.error || 'Failed to verify your email. Please request a new verification link.'
      }
    } catch (error) {
